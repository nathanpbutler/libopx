...
class Program
{
    static async Task<int> Main(string[] args)
    {
        ...
        var filterCommand = await CreateFilterCommand();
        var extractCommand = await CreateExtractCommand();

        rootCommand.Add(filterCommand);
        rootCommand.Add(extractCommand);
        rootCommand.Add(new Option<bool>("--version")
        {
            Aliases = { "-v" },
    ...
    /// <summary>
    /// Creates the filter command with its options and arguments.
    /// </summary>
    /// <returns>A Command object representing the filter command.</returns>
    /// <exception cref="ArgumentException">Thrown if the input file does not exist.</exception>
    /// <exception cref="InvalidOperationException">Thrown if the input format is invalid.</exception>
    /// <exception cref="Exception">Thrown for any other errors during command execution.</exception>
    /// <remarks>
    /// This method sets up the filter command with options for input file, magazine number, rows, input format, line count, version, and verbose output.
    /// It also handles the parsing of the command line arguments and executes the filtering operation.
    /// </remarks>
    private static async Task<Command> CreateFilterCommand()
    {
        ...
        filterCommand.Options.Add(versionOption);
        ...
        filterCommand.SetAction(async (parseResult) =>
        {
            ...
            bool version = parseResult.GetValue(versionOption);
    ...
    /// It also handles the parsing of the command line arguments and executes the filtering operation.
    /// </remarks>
    private static async Task<Command> CreateExtractCommand()
    {
        ...
        extractCommand.Options.Add(versionOption);
